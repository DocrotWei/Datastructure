...
Vertex FindRoot(Disjset S, Vertex V){
    ...
    for(Root = V; Root > 0; Root = S[Root]); //找到根节点
    /*路径压缩*/
...
void MinHeap(Edge E, int Parent, int Size){
    ...
    temp = E[Parent];
    for(; ((Parent<<1)+ 1) < Size; Parent = Child){
        Child = (Parent << 1) + 1;
        if((Child != Size - 1) && (E[Child].weight) > E[Child+1].weight)
            Child++;
...
int GetEdge(Edge E, int CurrentSize){
    ...
    Temp = E[0];
    E[0] = E[CurrentSize - 1];
    E[CurrentSize - 1] = Temp;
    MinHeap(E, 0, CurrentSize-1);
    return CurrentSize-1; //返回最小位置
}
/*初始化最小堆*/
